...
std::unique_ptr<StrategyVector> CreateElementwiseStrategies(
    size_t instruction_id, const HloInstruction* ins,
    const StrategyMap& strategy_map, const ClusterEnvironment& cluster_env,
  ...
  auto strategies = CreateLeafStrategyVector(instruction_id, ins, strategy_map,
                                             leaf_strategies);
    ...
    switch (opcode) {
      ...
      case HloOpcode::kBitcast: {
        if (ins->shape() == ins->operand(0)->shape()) {
          strategies = CreateElementwiseStrategies(
              instruction_id, ins, strategy_map, cluster_env,
              only_allow_divisible, replicated_penalty, batch_dim_map,
              solver_option, depth_map, alias_map, pretrimmed_strategy_map,
              max_depth, leaf_strategies, associative_dot_pairs);
        } else {
